...
class Config():
    # TODO pythonのdataclassに移行する.(Anacondaにpython3.10が実装されてから) 
    def __init__(self):
        #self.columns = None # 
        #self.csv = None # 
        self.id  = None
...
def execute(c):
    ...
    c.cnt_overflow = model.cnt_overflow
    
    print('Overflow ={:.3g} '.format(c.cnt_overflow))

    if c.plot:
        model.plot()
        task.plot(c,Utest[c.NS0:],model.X[c.NS0:],Ypred[c.NS0:],Ytest[c.NS0:])
